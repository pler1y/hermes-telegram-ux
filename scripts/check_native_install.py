...
def probe():
    ...
    assert any("Use English" in section.content for section in render_system_prompt_sections(
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="hermes-ux-native-") as raw:
        ...
        if has_validator:
            run(cli + ["validate", str(source), "--json"], cwd=temp, env=env)
        ...
        run([sys.executable, str(ROOT / "scripts/check_native_install.py"), "--probe"], cwd=temp, env=env)
        run(setup + ["configure", "--language", "en"], cwd=temp, env=env)
        ...
        print(json.dumps({"official_validator": has_validator, "native_git_install": True,
