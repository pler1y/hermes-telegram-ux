...
| ZIP payload | This core's native installer accepts Git/Catalog sources, not a ZIP file. A verified ZIP is extracted into the same-ID directory, then managed through native validation and activation | [plugins_cmd.py:204](https://github.com/NousResearch/hermes-agent/blob/3c3ab69abb9b08683b5eb15b4e2b8be1198c875f/hermes_cli/plugins_cmd.py#L204), [local lifecycle harness](../scripts/check_native.py) |
...
Follow [MIGRATION-v2.md](MIGRATION-v2.md). Pinned installs require an explicit **40-character commit SHA** and a reviewed force reinstall; `--ref release/v2.0.0` is not accepted by the checked core. `--no-enable` suppresses enabling during installation but does not disable an already enabled plugin or stop a running Gateway.

The v2 candidate is not a final tag, GitHub Release, or Catalog publication. Code-level findings in this document do not substitute for install tests. Exact final candidate commit, new-install and old-install upgrade results belong to [VALIDATION-v2.md](VALIDATION-v2.md); prior `1.9.0-catalog.1` live records remain historical evidence.
