...
| `soft_wait` | false | 设为 true 时，省略即时接话与普通模型等待提示；实际工具操作、重要通知和长时间等待仍显示 |
...
默认接话在 Telegram 消息入口执行，不受 `status_delay_seconds` 或文字合并窗口推迟。`soft_wait=true` 时保持安静，直到实际工具操作、明确的公开进度、补充回执或长时间等待需要显示。上下文整理、结束、停止和审批状态仍优先显示。表情不会按计时器轮换；关闭 `status_emoji` 只关闭进度自动装饰，接话和菜单文案仍可包含表情。

`status_min_edit_seconds` 控制常规进度编辑频率。发送或编辑失败后，插件逐步延长重试间隔；Telegram 要求等待时，同一机器人的进度气泡、接话及“输入中”提示会遵守该等待时间。恢复后显示最新状态，不补发等待期间的旧进度。
