...
For stable version 1.8.3, use a [core supported by that release](https://github.com/pler1y/hermes-telegram-ux/blob/v1.8.3/docs/COMPATIBILITY.md). Confirm your model login and Telegram bot work before adding this plugin. The installer checks all guarded files against one tested baseline. This page covers ZIP lifecycle management; see [native installation](NATIVE-INSTALL.md) for Hermes-managed Git installs.

Download an edition and its matching `.zip.sha256` from the [1.8.3 stable release](https://github.com/pler1y/hermes-telegram-ux/releases/tag/v1.8.3):

- `hermes-telegram-ux-1.8.3-en.zip` for English.
- `hermes-telegram-ux-1.8.3-zh.zip` for Chinese.

This release supports only the five reviewed core baselines in the [compatibility table](COMPATIBILITY.md).
